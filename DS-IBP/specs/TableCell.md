...
| Закрепление | клик по `.th__pin` переключает `.th--pinned`/`.tc--pinned` у всей колонки (фон Pinned, `pin`↔`pin-filled`, `aria-pressed`); событие `columnpin` с `{ column, pinned }`. При горизонтальном скролле удержание JS-ом (`tbl-pin.js`, `transform: translateX`): left-pinned — у левого края (или закреплённой левее), right-pinned (хвостовой блок) — у правого края; примыкающие сепараторы прилипают вместе с колонкой и получают `data-pin-side` (заливка `--bgtable-pinned`). `transform` не сужает ширину прокрутки — `min-width: max-content` у строки **прокручиваемой** таблицы (`.tbl--scroll .tbl__row`, `.dtable__body .tbl__row`) держит полный скролл-диапазон. CSS `position:sticky` в grid не работает |
...
- **Закрепление**: клик по `.th__pin` → фон Pinned у всей колонки, pin → pin-filled, `aria-pressed`; удержание при скролле JS-ом (`tbl-pin.js`): left-pinned — `translateX = max(0, scrollLeft − ширина_незакреплённых_слева)` (упирается в левый край или закреплённую левее), right-pinned (хвостовой блок) — у правого края; примыкающий сепаратор прилипает с колонкой и красится `--bgtable-pinned` (`data-pin-side` на сепараторе, по факту прилипания, а не по transform). `transform` не сужает ширину прокрутки — `min-width: max-content` у строки **прокручиваемой** таблицы (`.tbl--scroll .tbl__row`, `.dtable__body .tbl__row`) (иначе полоса прокрутки «не доезжает» и закреплённая перекрывает соседние слева). Не переставляет колонку; без hover-затемнения шапки.
...
| `.tc--accent` / `--pinned` | фон колонки (акцент / закреплённая); pinned удерживается у края рантаймом `tbl-pin.js` (JS-sticky) |
...
| `.th__pin` / `.th--pinned` | закрепление колонки (интерактив — `tbl-pin.js`); закреплённая — pin-filled, фон Pinned, удержание JS-ом (left/right по хвостовому блоку) |
