...
- **Поведение · порядок колонок** — только перетаскивание: перенос за **подпись** шапки (`.th--movable .th__label`, порог 4px), колонка приглушается (`.th--dragging`/`.tc--dragging`), место вставки — линия `.tbl__guide` в тоне `--primary`. Закреплённые колонки исключаются. Кнопок и пунктов меню, дублирующих перенос, у колонки нет.
...
- **Порядок колонок**: только drag за подпись (порог 4px), место вставки — ближайшая граница по центрам колонок; закреплённые исключены.
...
| `.th--movable` / `.th--dragging` / `.tc--dragging` | перенос колонки за подпись; колонка приглушена во время переноса |
