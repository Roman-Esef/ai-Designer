...
## Состав групп (эталон)
Канонический набор групп и пунктов меню (названия + иконки) — источник истины для сборки меню. Порядок групп настраивается данными (в демо — порядок массива `BLOCKS` в `nav-panel.page.js`).
...
| Текущий портфель ДИД | Текущий портфель ДИД `current-depo` · CF СБИ `cash-flow` · Корпоративные запросы `corporate-transactions` |
| Отчеты | Отчёты 1C/Navision `reports-1-c` · Реестр выписок `registry` · Расчет резерва `reserve` · Расчет FV `calclate-fv` · RWA `rwa` (canDisable) |
| Клиенты и задачи | Клиенты `factory-01` · Поиск клиентов в ЕПК `client-search` · Задачи `tasks` (Badge) |
| Администрирование | Администрирование `admin-panel-settings` · Администратор ЦКП `admin-ckp` |

## Роли
Роль определяет доступный набор групп меню; каждая группа доступна целиком (**full**) или частично (**partial** — только часть пунктов группы). Модель: `role → { groupId: 'full' | [itemId, …] }`; недоступные группы и пункты не рендерятся. Набор групп/пунктов по ролям задаётся данными продукта (в демо рендерится весь состав).

## Соответствие главной странице (отложено)
Каждая группа и каждый пункт меню соответствуют группе и тайлу стартовой главной страницы (зеркально 1:1); исключение — пункт «Поиск клиентов в ЕПК» (тайла нет). Пункт меню без тайла на главной странице — ошибка. Правило и валидация закрепляются при создании стартовой страницы (отложено, 01.09.2026).
