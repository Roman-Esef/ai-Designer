...
**Инварианты:** max-width 320px фиксирован на всех размерах; чип сжимаем (`min-width: 0`) — в чужой flex-строке уступает место и усекает подпись внутри плашки, а не вылезает за границу контейнера; hover/selected — только у Edit, Readonly не реагирует на курсор.
...
**Оси:** тип содержимого (текст/числа/дерево/чипы/контролы) · сортировка (none→asc→desc→none, активна максимум одна колонка) · закрепление (тогл `.th__pin` из коробки; JS-sticky: left-pinned у левого края, right-pinned — хвостовой блок у правого; сепаратор прилипает с примыкающей закреплённой — заливка `--bgtable` в покое, `--bgtable-pinned` при прилипании; `min-width: max-content` у строки **прокручиваемой** таблицы (`.tbl--scroll .tbl__row`, `.dtable__body .tbl__row`) — transform не сужает ширину прокрутки, иначе закреплённая перекрывает соседние слева на свою ширину).
