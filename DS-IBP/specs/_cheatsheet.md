...
<script>
  document.getElementById('cf-chart').appendChild(DSChart.make({
    type: 'combo', size: 'l',
    title: 'Движение средств по месяцам', subtitle: 'руб.',
...
**Оси:** тип (edit/readonly) · стиль (fill/outline) · размер (L/M/S/XS) · тон (нейтральный StSystem / любая рампа Local, вкл. solid-серия) · ведущий элемент (иконка/маркер/аватар) · действие (крестик ИЛИ шеврон — взаимоисключающе).
**Инварианты:** max-width 320px фиксирован на всех размерах; hover/selected — только у Edit, Readonly не реагирует на курсор.
...
css: `styles/layout.css` · deps: [nav-panel, breadcrumbs, spacing] · 1.007
**Оси:** режим навигации (rail 56 / drawer оверлей / fixed 272) · тип контента (тайлы .grid12 / таблица во всю ширину / карточка сущности).
**Инварианты:** между панелью и рабочей областью отступа нет; между крошками и контентом отступа нет; поля контентной области — 24px слева/справа и 24px снизу, сверху 0; зона крошек 44px и её поля 24px принадлежат компоненту Breadcrumbs, экран их не задаёт; крошки и контент выровнены по одной вертикали; **у экрана горизонтального скролла не бывает никогда — адаптивная вёрстка закладывается при проектировании, объект за границей экрана — баг**; исключение — контейнер со скроллом в собственной раскладке (тело таблицы, отдельный тайл), не сама рабочая область; скроллится только контент — крошки sticky, панель fixed на 100vh со своим внутренним скроллом списка; сетка не дублируется — .grid12/.col-N из Spacing.
...
**Оси:** режим (rail/drawer/fixed) · элементы (burger/item/item+badge/footer/divider, каждый в двух обликах Rail/Drawer).
...
**Классы:** `.nav` · `.nav--rail` / `.nav--drawer` / `.nav--fixed` · `.nav__top` · `.nav__burger` / `.nav__pin` · `.nav__list` · `.nav__block` · `.nav__block-label` · `.nav__item` · `.nav__item--selected` / `.nav__item--disabled` · `.nav__ico` / `.nav__label` / `.nav__badge` · `.nav__footer` / `.nav__user` / `.nav__logout`
...
Главная навигация приложения слева. Три режима: `nav--rail` (56px, иконки, тултип по hover, без тени), `nav--drawer` (272px, оверлей, тень Shadow4.0_modalform), `nav--fixed` (272px, закреплён, шов справа). Список = «Главная» + navigation-block'и (заголовок-секция + пункты), внизу футер: строка пользователя — ссылка на личный кабинет + кнопка выхода. Иконки пунктов — глифы раздела Menu (specs/Icons.md).
...
**Оси:** toolbar (вкл/выкл) · массовые действия (Footer) · настройка колонок (Modal --w4) · сохранённые фильтры (Split Button меню пресетов, см. TableFilter).
**Инварианты:** Table добавляет только обёртку/тулбар/липкую шапку — строки и ячейки строит TableCell; Disabled у таблицы нет.
**Классы:** `.dtable` · `.dtable__toolbar` / `-left` / `-right` · `.dtable__title` / `__count` · `.dtable__body` · `.dtable--scrolled` · `.dtable__footer` · `.dtable__empty` / `-title` / `-text` · `aria-busy="true"` · `.tbl` / `.tbl__row` / `.th` / `.tc` · `.pgn-row` / `.pgn-bulk`
**Диагностика:** «Тень под шапкой видна без скролла» → нужен класс `.dtable--scrolled`, навешиваемый по факту scrollTop>0 · «Таблица выглядит disabled целиком» → у Table такого состояния нет, приглушать нужно точечно (тулбар/инструменты)
...
**Из коробки:** подключить `scripts/ds-table.js` — самоинициализация по `.dtable__body` (scroll + ResizeObserver), выставляет `.dtable--scrolled` (тень под липкой шапкой при вертикальном скролле). Императивно на динамически вставленный узел — `DSTable.bind(bodyEl)` или `DSTable.bindAll(root)`. **Изменение ширины колонки работает из коробки** — ручка `.th__resize` стоит в каждой `.th`, скрипт `scripts/tbl-resize.js` входит в `scripts/ds.js` (делегирование на document, инициализация не нужна). Reorder/pin/sort, выбор строк и bulk-режим — модель данных на стороне потребителя, из коробки не даём. Отдельная колонка «Действие» — кнопки видны всегда (не по hover, в отличие от `.tc__hidden`); действие массового удаления в `.pgn-bulk` обязательно дублируется на уровне строки в этой колонке.

Полная анатомия: specs/Table.md.

## TableCell
css: `styles/table-cell.css` · js: `scripts/ds-table.js` · deps: [checkbox, chip, icon-button, button, input, dropdown-list, tooltip]
**Оси:** тип содержимого (текст/числа/дерево/чипы/контролы) · сортировка (none→asc→desc→none, активна максимум одна колонка).
**Инварианты:** интерактивные состояния — на строке, служебные — на ячейке; Disabled у таблицы нет.
**Классы:** `.tc` · `.tc--right` / `--center` · `.tc--numbers` · `.tc--accent` / `--pinned` · `.tbl__row--hover` / `--focus` / `--selected` · `.tc--hover` / `--focus` / `--selected` · `.tc--error` / `--error-bg` / `--skeleton` · `.tc--edited` · `.tc__row` · `.tc__text` / `--truncate` · `.tc__prefix` / `__postfix` · `.tc__icon` / `--lead` · `.tc__empty` · `.tc--wrap` · `.tc__body` / `__subtext` · `.tc--tree` + `--tc-level` · `.tc__twisty` / `--leaf` · `.tc__controls` · `.tc__hidden` · `.tc--input` · `.tc--separator` / `.th--separator` · `.tc__spark` · `.sk-line--caption` · `.th` · `.th__label` · `.th__tools` · `.th__sort` / `.th--sorted` · `.th__pin` / `.th--pinned` · `.th__resize` / `.th--resizing` · `.th--movable` / `.th--dragging` / `.tc--dragging` · `.tbl__guide` / `.tbl--resizing` / `.tbl--reordering` · `.th--select` / `.tc--select` · `.th__action` · `aria-sort` / `aria-pressed` / `aria-busy` / `aria-expanded` · `.tbl` / `.tbl--scroll` / `.tbl__row`
**Диагностика:** «Hover подсвечивает только одну ячейку, а не строку» → фон должен задаваться через `.tbl__row:hover > .tc`, не точечным классом на ячейке · «Ширина колонки выбора скачет между шапкой и строками» → класс `.th--select`/`.tc--select` должен стоять на всех ячейках колонки одинаково
...
**Оси:** позиция (12) · тип (main/error) · триггер (hover 400мс/focus мгновенно) · усечение (`data-tooltip-truncated="only"`).
