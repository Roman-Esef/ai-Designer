...
- **Порядок колонок**: drag за подпись шапки (порог 4px) переставляет колонку к ближайшей границе между центрами; разделители и закреплённые не переносятся. Логика — общий `scripts/tbl-reorder.js` (в составе `ds.js`), делегирование на document, инициализации на экране не требует; событие `columnreorder` на `.tbl` с `{from, to}`.
